...
//    	}
//    	
//    	if (fname.substring(0, 10).equals("jar:file:/")) {
			...
			if ("v".equals(str)) {// 頂点座標
				...
				for (i = 0; i < num; i++) {
					...
					if (val[0] > 0)
						...
						xvp.add(vp.size() + val[0] + 1);
					if (val[1] > 0)
						xvt.add(val[1]);
					else if (val[1] < 0)
						xvt.add(vt.size() + val[1] + 1);
					if (val[2] > 0)
						xvn.add(val[2]);
					else if (val[2] < 0)
						xvn.add(vn.size() + val[2] + 1);
				}
...
//		File mfile = new File(filePath + str);
		...
		try {
			InputStream in = getClass().getResourceAsStream(filePath + str);
			mtlBR = new BufferedReader(new InputStreamReader(in));
	...
	private void setv(String str, int[] v) {
		...
		for (i = 0; i < 3; i++) {
			for (q = p; q < str.length() && str.charAt(q) != '/'; q++)
				;
			if (q > p)
				v[i] = Integer.parseInt(str.substring(p, q));
			p = q + 1;
			if (p >= str.length())
				return;
		}
